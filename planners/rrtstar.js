export function runRRTStar(grid, start, goal, params) {
  const t0 = performance.now();

  const rows = grid.length;
  const cols = grid[0].length;
  const { stepSize, rewireRadius } = params;
  ...
  const cost = new Map(); // node key -> cost
  parents.set(`${start.r},${start.c}`, null);
  cost.set(`${start.r},${start.c}`, 0);

  let visited = [start];
  let openSets = [[start]]; // for animation
  ...
  function getNearNodes(newNode) {
    const near = [];
    for (let node of tree) {
      if (distance(node, newNode) <= rewireRadius) {
        near.push(node);
      }
    }
    return near;
  ...
  const maxIterations = 2000; // or from params?
  ...
  for (let iter = 0; iter < maxIterations; iter++) {
    ...
    if (isValid(newNode) && isLineClear(nearest, newNode)) {
      // Find best parent
      const nearNodes = getNearNodes(newNode);
      let bestParent = nearest;
      let bestCost = cost.get(`${nearest.r},${nearest.c}`) + distance(nearest, newNode);

      for (let near of nearNodes) {
        if (isLineClear(near, newNode)) {
          const newCost = cost.get(`${near.r},${near.c}`) + distance(near, newNode);
          if (newCost < bestCost) {
            bestParent = near;
            bestCost = newCost;
          }
      ...
      // Add newNode
      tree.push(newNode);
      parents.set(`${newNode.r},${newNode.c}`, bestParent);
      cost.set(`${newNode.r},${newNode.c}`, bestCost);
      visited.push(newNode);
      openSets.push([...tree]);

      // Rewire
      for (let near of nearNodes) {
        if (near !== bestParent && isLineClear(newNode, near)) {
          const rewireCost = bestCost + distance(newNode, near);
          if (rewireCost < cost.get(`${near.r},${near.c}`)) {
            parents.set(`${near.r},${near.c}`, newNode);
            cost.set(`${near.r},${near.c}`, rewireCost);
          }
        }
      }
      ...
      if (distance(newNode, goal) <= stepSize && isLineClear(newNode, goal)) {
        const goalCost = bestCost + distance(newNode, goal);
        if (!parents.has(`${goal.r},${goal.c}`) || goalCost < (cost.get(`${goal.r},${goal.c}`) || Infinity)) {
          parents.set(`${goal.r},${goal.c}`, newNode);
          cost.set(`${goal.r},${goal.c}`, goalCost);
          visited.push(goal);
